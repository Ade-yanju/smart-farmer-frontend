...
import { signInWithGoogle, googleErrorMessage } from "../googleAuth";
...
export default function Signup() {
  ...
  const handleGoogleSignup = async () => {
    ...
    setError("");
    try {
      // Creates the users/{uid} document on first sign-in (with the referral
      // code if one was typed). Google emails are already verified, so no
      // verification email is sent for this flow.
      await signInWithGoogle(referralCode.trim());
      navigate("/dashboard");
    } catch (err) {
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
        <div className="form-wrapper" style={{ width: "100%", maxWidth: "440px", padding: "3rem" }}>
          ...
          <Reveal>
            ...
            <button className="social-btn" onClick={handleGoogleSignup} disabled={googleLoading} style={{ width: "100%", padding: "0.875rem", background: "transparent", border: `1px solid ${styles.border}`, borderRadius: "10px", color: "white", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.75rem", cursor: googleLoading ? "not-allowed" : "pointer", transition: "all 0.2s", fontWeight: 500, opacity: googleLoading ? 0.6 : 1 }}>
